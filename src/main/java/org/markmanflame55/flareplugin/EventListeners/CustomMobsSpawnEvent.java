...
public class CustomMobsSpawnEvent implements Listener {
            ...
            if (e.getEntity().getKiller() != null) {
                ...
                if (e.getEntity().getCustomName() != null) {
                    ...
                    if (e.getEntity().getScoreboardTags().contains("berserk")) {
                        ...
                        float chance = random.nextFloat();
                        if (chance <= 0.75f) {
                            e.getDrops().add(GrimReaperArmorUtil.giveUndeadChains());
                        }
                    }
    ...
    // WITHERSKELETONS============================================================================================================================
    @EventHandler
    public void onReplacedMobSpawn(EntitySpawnEvent e) {
        ...
        if (e.getEntity() instanceof Spider && !(e.getEntity() instanceof CaveSpider)) {
            ...
            if (chance <= 0.25f) {
                ...
                e.getEntity().addPassenger(CustomMobsUtil.summonVampire(e.getLocation(), e.getEntity()));
            }
        }
    }
    @EventHandler
    public void onWitherSkeletonDeath(EntityDeathEvent e) {
        ...
        if (e.getEntity() instanceof WitherSkeleton && e.getEntity().getScoreboardTags().contains("vampire")) {
            e.getDrops().clear();
            e.getEntity().getWorld().playSound(e.getEntity().getLocation(), Sound.ENTITY_GHAST_DEATH, 1.0f, 1.3f);
            Random random = new Random();
            float chance = random.nextFloat();
            if (chance <= 0.5f) {
                e.getDrops().add(VampireSword.giveVampireFang());
            }
        }
    }
    @EventHandler
    ...
    @EventHandler
    public void onWitherHitOrDamage(EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof WitherSkeleton vampire && vampire.getScoreboardTags().contains("vampire")) {
            if (((WitherSkeleton) e.getDamager()).getHealth()+1 > 20) {
                ((WitherSkeleton) e.getDamager()).setHealth(20);
            } else {
                ((WitherSkeleton) e.getDamager()).setHealth(((WitherSkeleton) e.getDamager()).getHealth()+1);
            }
        }
        if (e.getEntity() instanceof WitherSkeleton vampire && vampire.getScoreboardTags().contains("vampire")) {
            e.getDamager().getWorld().playSound(e.getEntity().getLocation(), Sound.ENTITY_WARDEN_HURT, 0.4f, 1.3f);
        }
    }
